...
def get_morgan_fp(mol, n=2, bits=1024):
...
def prepare_df_chemeleon(df_original):
    ...
    It performs all necessary processing to make it usable in the code, including :
                 ...
                 - standardize the complexes with 2 identical ligands to AAB representation
                 - calculates the CheMeleon fingerprints and adding them in a Fingerprint column 
                 - renames the columns
                 - associates an ID to each complex (necessary for indicing functions)
                 - calculates the pIC50 drom the IC50 column
    ...
    Args:
        df: the dataframe 
        r (int): the radius of the CheMeleon fingerprint
        bits (int): the nBits value of the CheMeleon fingerprint
    
    returns:
        df: the processed dataframe with CheMeleon Fingerprints 
    """
    ...
    df['Desc1'] = list(fp_generator(df["MOL1"].tolist()))
    df['Desc2'] = list(fp_generator(df["MOL2"].tolist()))
    df['Desc3'] = list(fp_generator(df["MOL3"].tolist()))
...
# def get_indices_chemeleon_2(df, CV, sizes=(0.9, 0.1), seeder=0):
...
def get_indices_chemeleon_DOI(df, CV, sizes=(0.9, 0.1), seeder=0):
    ...
    random.seed(seeder)
    random.shuffle(DOI) # Randomly shuffle unique DOIs
    test_size = 1/CV*len(DOI)
    train_val_size = len(DOI)-test_size
    train_size = sizes[0] * train_val_size
    val_size = sizes[1] * train_val_size
    ...
    DOI_test = np.array_split(DOI, round(CV))
...
def get_indices_chemeleon_scaff(mols_x, CV, sizes=(0.9, 0.1), seeder=0):

    scaff_dict = scaffold_to_smiles(mols_x, use_indices=True) # We extract the scaffolds and the indices of the complexes having each scaffold
    scaffolds = list(scaff_dict.keys()) # Define a list of the possible scaffolds
    k = 1 # Iteration count
    random.seed(seeder)
    random.shuffle(scaffolds) # Randomly shuffle scaffolds

    test_size = 1/CV * len(scaffolds)
    train_val_size = len(scaffolds)- test_size
    train_size = sizes[0] * train_val_size
    val_size = sizes[1] * train_val_size
    ...
    scaff_test = np.array_split(scaffolds, CV)
    ...
    for i in range(CV):
        ...
        print(f'train length : {len(train)}, scaffolds : {len(scaff_train)} | val length : {len(val)}, scaffolds : {len(scaff_val)} | test length : {len(test_CV[i])}, scaffolds : {len(scaff_test[i])}')
