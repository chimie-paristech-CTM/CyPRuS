...
from .bayesian_opt import objective_xgboost, objective_rf, objective_knn, objective_mlp, objective_chemeleon
...
def get_optimal_parameters_gnn(df, logger, max_eval=32, descriptors=False):
    """
    Get the optimal descriptors for random forest (descriptors) through Bayesian optimization.
    ...
    "hidden_dim": hp.choice("hidden_dim", [200, 300, 400]),
    "dropout": hp.choice("dropout",  [0, 0.05, 0.1, 0.15, 0.2]),
    #"batch_size": hp.choice("batch_size", [16, 32, 64])
    }

    if descriptors == True :
        optimal_parameters = bayesian_opt(df, logger, space, objective_chemeleon, model_class=None, max_eval=max_eval)
    else : 
        optimal_parameters = bayesian_opt(df, logger, space, objective_chemeleon, model_class=None, max_eval=max_eval)

    if logger : 
        logger.info(f'Optimal parameters for CheMeleon GNN : {optimal_parameters}')
        logger.info(f'Optimal parameters for CheMeleon GNN : {space_eval(space, optimal_parameters)}')

    return optimal_parameters


def get_cross_val_accuracy_gnn(df, logger, n_fold, parameters, split_dir=None, descriptors=False):
    """
    Get the xgboost (fingerprints) accuracy in cross-validation.
    ...
    params = {
        "n_layers": space["n_layers"][parameters["n_layers"]],
        "hidden_dim": space["hidden_dim"][parameters["hidden_dim"]],
        "dropout": space["dropout"][parameters["dropout"]],
    }
    
    rmse, mae = cross_val_chemeleon(df, n_fold, logger, hparams=params, split_dir=split_dir)
    
    if logger : 
        logger.info(f'{n_fold}-fold CV RMSE and MAE for CheMeleon : {rmse} {mae}')
        logger.info(f'Parameters used: {parameters}')
